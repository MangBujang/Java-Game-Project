...
// Jalankan fungsi ini tepat saat pemain menekan tombol "Enter" atau klik "Mulai Game" setelah input nama
function saveNewSlotToDatabase(slotId, inputName) {
    const newSaveData = {
        id: slotId,
        saveName: inputName, // Mengisi kolom SAVE_NAME di database
        playerLevel: 1,
        selectedDragonName: "None"
    };
    ...
    .then(res => res.json())
    .then(data => {
        console.log("Save state baru berhasil disimpan ke tabel SAVE_SLOT!");
        loadAllSlotsFromDatabase(); // Update list slot global
        gameState = "PLAYING";
    })
    .catch(err => console.error("Gagal membuat save state baru:", err));
}

function saveNewSlotToDatabase(slotId, inputName) {
    const newSaveData = {
        id: slotId,          // Nomor slot (1, 2, atau 3)
        saveName: inputName, // Nama yang diketik user (akan masuk kolom SAVE_NAME di H2)
        playerLevel: 1,
        selectedDragonName: "None"
    };

    // Kirim data menggunakan POST ke REST Controller Spring Boot
    fetch("/api/slots", {
        method: "POST",
        headers: {
    ...
    .then(data => {
        console.log("Data berhasil disimpan di tabel SAVE_SLOT:", data);
        
        // Panggil fungsi refresh data dari menu-save-slot.js agar UI terupdate
        if (typeof loadAllSlotsFromDatabase === "function") {
            loadAllSlotsFromDatabase();
        }
        
        // Setelah sukses menyimpan ke database H2, ganti layar ke gameplay utama!
        gameState = "PLAYING"; 
    })
    .catch(err => {
        console.error("Error database:", err);
        alert("Gagal menyimpan progres ke database H2 server!");
    });
}